...
class RealDIADAC:
    ...
    def encode(self, audio):
        ...
        Args:
            audio: Audio tensor [samples] or [1, samples] at any sample rate
        ...
        # Resample to 44.1kHz if needed (DAC expects 44.1kHz)
        if audio.shape[-1] != 44100:
            # Calculate current sample rate based on audio length
            # This is a rough estimate - in practice you'd know the original sample rate
            current_sr = 16000  # Assuming 16kHz from SemantiCodec
            audio = torchaudio.functional.resample(audio, current_sr, 44100)
