...
class PodcastGenerator:
    ...
    def __init__(self, openai_api_key: Optional[str] = None, seed: Optional[int] = None):
        ...
        try:
            ...
            # Define speaker tokens (just as strings, do not add to tokenizer)
            self.speaker_tokens = {
                '0': '[SPK0]',
                '1': '[SPK1]'
            }
...
Remember: Aim for {target_tokens} tokens total across {target_turns} dialogue turns!"""
    ...
    def generate_semantic_tokens_no_reference(self, dialogue: List[Dict[str, str]]) -> List[np.ndarray]:
        """
        Generate semantic tokens for each turn independently, but prepend a speaker ID token for consistency.
        """
        print("Generating semantic tokens with explicit speaker ID tokens for consistency...")
        ...
        for i, turn in enumerate(dialogue):
            ...
            text = turn["text"]
            role = turn["role"]
            # Prepend speaker token (just as a string)
            speaker_token = self.speaker_tokens.get(role, '[SPK0]')
            text_with_speaker = f"{speaker_token} {text}"
            semantic_tokens = self.text_to_semantic.generate_semantic_tokens_simple(text_with_speaker)[0].astype("int16")
            semantic_tokens_list.append(semantic_tokens)
            print(f"    Done in {time.time() - start:.1f}s")
        print(f"Total time: {time.time() - total_start:.1f}s")
        return semantic_tokens_list

    def semantic_tokens_to_audio(self, tokens_list: List[np.ndarray], dialogue: List[Dict[str, str]], speaker_reference_audio: Optional[Dict[str, str]] = None) -> torch.Tensor:
        """
        Convert semantic tokens to audio using MoonCast's pipeline.
        If speaker_reference_audio is provided (dict: role -> audio file path), use reference audio for speaker consistency.
        Otherwise, fall back to no-reference generation.
        """
        print("Converting semantic tokens to audio (with optional speaker reference audio)...")
        audio_segments = []
        for i, tokens in enumerate(tokens_list):
            ...
            if isinstance(tokens, np.ndarray):
                tokens = torch.tensor(tokens, dtype=torch.long, device="cuda" if torch.cuda.is_available() else "cpu")
            if tokens.dim() == 1:
                tokens = tokens.unsqueeze(0)
            start_time = time.time()
            role = dialogue[i]["role"]
            use_ref = False
            ref_wav = None
            if speaker_reference_audio and role in speaker_reference_audio:
                ref_path = speaker_reference_audio[role]
                if os.path.exists(ref_path):
                    ...
                    use_ref = True
            if use_ref:
                print(f"    Using reference audio for speaker {role}: {ref_path}")
                # Use detokenize with reference audio, no reference tokens
                gen_speech_fm = detokenize(self.audio_detokenizer, tokens, ref_wav, None)
            else:
