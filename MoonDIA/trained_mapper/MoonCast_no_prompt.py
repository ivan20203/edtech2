...
Remember: Aim for {target_tokens} tokens total across {target_turns} dialogue turns!"""
    ...
    def generate_semantic_tokens_no_reference(self, dialogue: List[Dict[str, str]]) -> List[np.ndarray]:
        ...
        This version generates semantic tokens for each turn independently (no growing prompt).
        """
        print("Generating semantic tokens without voice cloning (no prompt, per-turn only)...")
        ...
        for i, turn in enumerate(dialogue):
            ...
            text = turn["text"]
            # Generate semantic tokens for this turn only (no context)
    ...
    def semantic_tokens_to_audio(self, tokens_list: List[np.ndarray], dialogue: List[Dict[str, str]]) -> torch.Tensor:
        ...
        Args:
            tokens_list: List of semantic token sequences (one per turn)
